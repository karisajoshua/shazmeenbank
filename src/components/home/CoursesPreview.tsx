...
const CourseCard = ({
  ...
  return <div className="premium-card shadow-premium overflow-hidden card-hover">
      <div className="h-52 overflow-hidden">
        <img src={imageSrc} alt={title} className="w-full h-full object-cover transition-transform duration-700 hover:scale-105" />
      </div>
      <div className="p-8">
...
const CoursesPreview = () => {
  ...
  return <section className="section-padding bg-shazmeen-gray bg-opacity-30">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {featuredCourses.map(course => <CourseCard key={course.slug} title={course.title} description={course.description} level={course.level} duration={course.duration} imageSrc={course.imageSrc} slug={course.slug} />)}
        </div>
      </div>
    </section>;
};
